...
| --- | --- | --- | --- |
| `SCHEDULE_FREQUENCY` | Cron schedule for automated workflow runs | `0 */2 * * *` | Every 4 hours: `0 */4 * * *`, Daily at midnight UTC: `0 0 * * *`, Mondays at 9 AM UTC: `0 9 * * MON` |
| `DIRECTORY_TO_MONITOR` | Directory path to scan for changes | `.github` | `config`, `infra`, `.github/workflows`, `terraform` |
| `COMMIT_MESSAGE_PREFIX` | Text prefix for commit messages | `Update` | `chore: Sync`, `[Auto]`, `ci: Update` |
| `AUTO_MERGE_LABEL` | Label applied to PRs for identification | `auto-merge` | `auto-merge-config`, `automation`, `auto` |
| `BASE_BRANCH` | Target branch for pull requests | `main` | `develop`, `staging`, `production` |
